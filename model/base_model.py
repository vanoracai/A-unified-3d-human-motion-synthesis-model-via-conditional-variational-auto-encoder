...
class BaseModel():
    def __init__(self, opt):
        ...
        self.load_dir = os.path.join(opt.checkpoints_dir, opt.load_model_name)
        self.loss_names = []
        self.model_names = []
        self.visual_names = []
        self.value_names = []
    ...
    def setup(self, opt):
        ...
        if self.isTrain:
            self.schedulers = [base_function1.get_scheduler(optimizer, opt) for optimizer in self.optimizers]
        if not self.isTrain or opt.continue_train:
            self.load_networks(opt.which_iter,rank=opt.rank)
    ...
    def get_current_dis(self):
        ...
        for i in range(1):
            for j, name in enumerate(self.value_names):
                if isinstance(name, str):
                    dis_ret[name+str(i)] =util.tensor2array_1d(value[i][j].data)
    ...
    def save_networks(self, which_epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                # if not self.opt.dist or (self.opt.dist and self.opt.rank % self.opt.num_gpus == 0):
    ...
    # load models
    def load_networks(self, which_epoch, rank=-1):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                net = getattr(self, 'net_' + name)
                if self.opt.dist and self.opt.rank % self.opt.num_gpus == 0:
                    # net = net.module
                    pretrained_dict = torch.load(
                            path, map_location=lambda storage, loc: storage.cuda(torch.cuda.current_device()))
                else:
                ...
                if not self.isTrain:
                    net.eval()
        if rank<=0:
            print("load %s network" % which_epoch)
