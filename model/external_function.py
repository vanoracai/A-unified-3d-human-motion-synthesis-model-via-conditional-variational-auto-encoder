...
def cal_gradient_penalty(netD, real_data, fake_data, actions=None,type='mixed', constant=1.0, lambda_gp=10.0):
    ...
    if lambda_gp > 0.0:
        ...
        disc_interpolates = netD(interpolatesv, actions)
...
#     else:
#         _ls = self.softmax_and_cross_entropy(preds, labels)
#         return (norm_loss * _ls).sum()

def softmax_and_cross_entropy(logits, labels):
    n,c,t = logits.size()
    return -(torch.nn.Softmax(dim=1)(labels) * torch.nn.LogSoftmax(dim=1)(logits)).sum()/(n*c*t)
